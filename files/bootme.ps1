...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_influxdb"
# Usage:
# <powershell>
...
#</powershell>
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_influxdb" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_influxdb" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_influxdb"
  }
